...
def sigclust(X, mc_iters=100, method=2, verbose=True, scale=True,
    ...
    if verbose:
        print("\nThe %d variances for simulation have\nmean: %f\n"
              "standard deviation: %f." % (X.shape[1], np.mean(sim_vars),
              np.std(sim_vars)))
...
def recclust(X, threshold=.01,
    ...
    "ids" is a numpy array of keys to use to record the cluster elements themselves.
...
def comp_sim_vars(eig_vals, bg_noise_var, method, p_op, ngrains):
    """
    Compute variances for simlulation given sample variances, bacground noise varance.
...
def comp_sim_tau(rsrtd_vals, bg_noise_var, p_op, ngrains):
    """
    Find tau to preserve trace of sample cov matrix in the simulation cov matrix.
